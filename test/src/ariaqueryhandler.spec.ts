...
import {AwaitableIterable} from '../../lib/cjs/puppeteer/common/types.js';
import utils, {Iterable} from './utils.js';

const getIds = async (elements: AwaitableIterable<ElementHandle<Element>>) => {
  return Iterable.map(elements, element => {
...
describeChromeOnly('AriaQueryHandler', () => {
  ...
  describe('queryAll', () => {
    it('should find menu by name', async () => {
      ...
      expect(await getIds(divs)).toEqual(['mnu1', 'mnu2']);
    });
  });
